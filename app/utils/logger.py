...
- Путь логов и режим работы определяется переменными BASE_PATH и APP_MODE.
...
from app.core.config import BASE_PATH, APP_MODE
...
try:
    ...
    fallback_path = "/tmp"
    print(f"[LOGGER INIT] ❌ Нет прав на {BASE_LOGS_PATH}, переключаюсь на {fallback_path}")
